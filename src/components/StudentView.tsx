...
import { TutoringRequest, UserProfile, handleFirestoreError, OperationType } from '../types';
...
export default function StudentView() {
  ...
  useEffect(() => {
    ...
    }, (error) => handleFirestoreError(error, OperationType.GET, 'users', auth));

    // Fetch student's requests
    if (!profile) return;
    const qRequests = query(collection(db, 'requests'), where('studentId', '==', profile.uid));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newRequest: Omit<TutoringRequest, 'id'> = {
      ...
      handleFirestoreError(error, OperationType.CREATE, 'requests', auth);
    }
  };
